...
class ApiTest(testtools.TestCase):
    ...
    def test_start_mysql_with_conf_changes(self):
        exp_msg = RpcMsgMatcher('start_mysql_with_conf_changes',
                                'updated_memory_size')
        self._mock_rpc_call(exp_msg)
        self.api.start_mysql_with_conf_changes('512')
        self._verify_rpc_call(exp_msg)

    def test_stop_mysql(self):
        exp_msg = RpcMsgMatcher('stop_mysql', 'do_not_start_on_reboot')
        self._mock_rpc_call(exp_msg)
        self.api.stop_mysql(do_not_start_on_reboot=False)
        self._verify_rpc_call(exp_msg)
